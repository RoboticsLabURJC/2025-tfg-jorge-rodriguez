...
class IMUSensor(object):
    """Reads gyroscope (angular velocity) data from sensor.other.imu.

    Attributes:
        gyro_x  -- angular velocity around the X axis (rad/s)  -> roll rate
        gyro_y  -- angular velocity around the Y axis (rad/s)  -> pitch rate
        gyro_z  -- angular velocity around the Z axis (rad/s)  -> yaw rate
    """
    ...
    def _on_imu_event(weak_self, event):
        ...
        if not self:
            return
        # event.gyroscope is a carla.Vector3D (rad/s)
...
def game_loop(args):
    ...
    try:
        ...
        # ---- CSV header now includes gyroscope columns ----
        writer_output.writerow(["image_name", "label", "throttle", "steer",
                                 "gyro_x", "gyro_y", "gyro_z"])

        DEBUG = args.debug

        drunk_enabled = args.drunk

        drunk = False
        drunk_start_time = None
        last_drunk_time = time.time()

        drunk_interval_sec = args.drunk_interval
        drunk_duration_sec = args.drunk_time
        ...
        while True:
            ...
            if DEBUG:
                print("DEBUG MODE")
                print(f"{controller._control.throttle}\n")
            ...
            if drunk_enabled and world.data_record:
                if not drunk and (current_time - last_drunk_time >= drunk_interval_sec):
                    drunk = True
                    drunk_start_time = current_time
                    last_drunk_time = current_time
                    steer = random.uniform(-drunk_range, drunk_range)
            
            if not world.data_record:
                drunk = False

            if drunk:
                if current_time - drunk_start_time >= drunk_duration_sec:
                    print(current_time - drunk_start_time)
                    drunk = False
                    recovering = True
                    recovering_time = time.time()
                else:
                    control = carla.VehicleControl()
                    control.throttle = controller._control.throttle
                    control.steer = steer
                    control.brake = controller._control.brake
                    control.hand_brake = controller._control.hand_brake
                    control.reverse = controller._control.reverse
                    control.gear = controller._control.gear

                    world.player.apply_control(control)
            if recovering and time.time() - recovering_time >= 1.2:
                recovering = False
            
            if image is not None and world.data_record and not drunk:

                steer_value = controller._control.steer

                bin_index = np.digitize(steer_value, bins) - 1
                ...
                if bin_counts[bin_index] < max_samples_per_bin:
                    ...
                    if controller._control.gear < 0:
                        throttle *= -1

                    # ---- Read gyroscope from IMU sensor ----
                    gyro_x = world.imu_sensor.gyro_x
                    gyro_y = world.imu_sensor.gyro_y
                    gyro_z = world.imu_sensor.gyro_z
