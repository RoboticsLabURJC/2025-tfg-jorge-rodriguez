...
class DualControl(object):
    def __init__(self, world, start_in_autopilot):
        ...
        self._parser.read('/home/jrguezg/2025-tfg-jorge-rodriguez/carla_tests/config_steering_wheel.ini')
        self._steer_idx = int(
    ...
    def parse_events(self, world, clock):
        ...
        if not self._autopilot_enabled:
            ...
            # in case it doesnt work uncomment this line
...
def return_label(steer_value, recovering, threshold=0.15):
    if recovering:
        return "recovering"

    if abs(steer_value) < threshold:
        return "straight"
    
    return "left turn" if steer_value <= -threshold else "right turn"
...
def game_loop(args):
    ...
    try:
        ...
        writer_output.writerow(["image_name", "label", "throttle", "steer"])
        ...
        recovering = False
        drunk_start_time = 0
        last_drunk_time = time.time()
        recovering_time = time.time()
        drunk_range = args.drunk_range
        was_recording = False

        # Bins configuration for balancing
        num_bins = 25
        ...
        while True:
            ...
            # --- RESTARTING TIMERS ---
            if world.data_record and not was_recording:
                last_drunk_time = current_time
                recovering = False

            was_recording = world.data_record
            ...
            # Takes current control

            current_control = controller._control
            steer_to_apply = current_control.steer
            
            # --- DRUNK MODE MANAGEMENT ---
            
            if not world.data_record:
                drunk = False

            if args.drunk and world.data_record:
                if not drunk and (current_time - last_drunk_time >= args.drunk_interval):
                    drunk = True
                    drunk_start_time = current_time
                    last_drunk_time = current_time
                    steer_drunk_steering = random.uniform(-drunk_range, drunk_range)  

                if drunk:
                    if current_time - drunk_start_time >= args.drunk_time:
                        drunk = False
                        recovering = True
                        recovering_time = current_time
                    else:
                        steer_to_apply = steer_drunk_steering
            
            # --- NOISE INJECTION MANAGEMENT ---

            if args.noise_injection and world.data_record:
                noise = np.random.normal(0, 0.05)
                steer_to_apply = np.clip(steer_to_apply + noise, -1.0, 1.0)

            # Resets recovering timer

            if recovering and (current_time - recovering_time >= args.recovering_timer):
                recovering = False

            # Applies control

            control_to_carla = carla.VehicleControl(
                    throttle = current_control.throttle,
                    steer = steer_to_apply,
            ...
            world.player.apply_control(control_to_carla)
            
            # --- DATASET RECORDING ---

            if image is not None and world.data_record and not drunk:

                bin_index = np.digitize(current_control.steer, bins) - 1

                bin_index = max(0, min(bin_index, num_bins - 1))

                if bin_counts[bin_index] < max_samples_per_bin:
                    ...
                    label = return_label(current_control.steer, recovering)

                    writer_output.writerow([
                        str(iteration) + '.png',
                        label,
                        controller._control.throttle * controller._control.gear,
                        current_control.steer
                    ])
...
def main():
    ...
    argparser.add_argument(
        '--drunk',
        action='store_true',
        help='Record dataset using drunk driver noise')
    argparser.add_argument(
        '--drunk_interval',
        default=15,
        type=float,
        help='How often is drunk driver triggered in frames') 
    argparser.add_argument(
        '--drunk_time',
        default=1.5,
        type=float,
        help='How many frames is drunk active')  
    argparser.add_argument(
        '--drunk_range',
        default=0.3,
        type=float,
        help='Maximum steering on drunk mode')
    argparser.add_argument(
        '--recovering_timer',
        default=1.2,
        type=float,
        help='Defines how much time after the drunk leaves the steering wheel is considered as recovering')
    argparser.add_argument(
        '--noise_injection',
        action='store_true',
        help='Record dataset using noise injection')
    argparser.add_argument(
        '--sync',
        action='store_true',
